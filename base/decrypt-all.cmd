...
if not files:
    print(f"[decrypt] no swf files under {target}")
    raise SystemExit(0)
...
for path in files:
    ...
    if kind == "plain":
        print(f"[skip] already plain: {path}")
        continue
    if kind != "wrapped":
        print(f"[skip] unknown format: {path}")
        continue
    out = unique_output(path, "decrypted")
    out.write_bytes(payload)
    print(f"[ok] decrypted: {path} -> {out}")
